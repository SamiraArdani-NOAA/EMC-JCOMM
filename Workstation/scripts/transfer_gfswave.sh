...
for cyc in ${cycles}
do
  echo "running transfer_gfswave.sh for $theDate ${cyc}" >> jcomm_checks.log
  ...
  then
    ...
    $SCP -o ConnectTimeout=300 -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null wave_NCEP_${theDate}${cyc}_prod_fc.grib2 ${USERpolar}:${polardir}/.wave_NCEP_${theDate}${cyc}_prod_fc.grib2 1>jcomm_copy_${cyc}.log 2>&1 
    OK=$?
    ...
    then
      ...
      $SCP -o ConnectTimeout=300 -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null ${USERpolar}:${polardir}/wave_NCEP_${theDate}${cyc}_prod_fc.grib2.MD5 ${workdir}/${theDate}/wave_NCEP_${theDate}${cyc}_prod_fc.grib2.MD5.polar
      OK=$?
...
cat jcomm_checks.log | mail -s "JCOMM2 checks" Deanna.Spindler@noaa.gov
